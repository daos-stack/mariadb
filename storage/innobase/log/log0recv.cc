			      ...
			      mlog_init_t::init* init = NULL)
{
	...
	for (const log_rec_t* recv : p->second.log) {
		...
		if (UNIV_UNLIKELY(!init && rec_was_skipped)) {
			rec_was_skipped = false;
			ut_ad(end_lsn == page_lsn);
			if (end_lsn != page_lsn)
				ib::warn() << "The last skipped log record LSN " << end_lsn
					<< " is not equal to page LSN " << page_lsn;
		}
